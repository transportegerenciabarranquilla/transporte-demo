...
export default function RegistroModulacionPage() {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      Promise.all([
        portalFetch(`/api/seguimiento?contratista=${encodeURIComponent(contratista)}&dt=${encodeURIComponent(dt)}&fecha=${encodeURIComponent(todayKey)}`, {
          cache: "no-store",
          signal: controller.signal,
        }),
        portalFetch(`/api/asistencias/buscar?contratista=${encodeURIComponent(contratista)}&dt=${encodeURIComponent(dt)}&fecha=${encodeURIComponent(todayKey)}`, {
          cache: "no-store",
          signal: controller.signal,
        }),
      ])
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      portalFetch(`/api/personas?cc=${encodeURIComponent(cedula)}&contratista=${encodeURIComponent(form.contratista)}`, {
        cache: "no-store",
        signal: controller.signal,
      })
        .then(async (response) => {
